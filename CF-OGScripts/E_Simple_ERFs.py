...
for actSubj in Subs:
    ...
    CombTrials.pick_types() # pick only meg channels (default)
    ...
    # # adapt this to your timing
    # ax.text(0.05,ymax,'S1')
    # ax.plot([1.3, 1.3], [ymin, ymax],'k')
    # ax.text(1.35,ymax,'S2')
    # ax.plot([2.6, 2.6], [ymin, ymax],'k')
    # ax.text(2.65,ymax,'Cue')
    
    fig.show()
    plt.savefig(str(homeDir)+'/processed/ERFs/VP'+str(actSubj)+'_simpleERFs.pdf')
    plt.close('all')

...
# saving the results
results_path = str(homeDir)+'/AWM4_data/processed/timepoints/locationS2'
...
for actSubj in sub_lst:
    ...
    plt.savefig(results_path + f'/sub{actSubj}_locationS2_mean_timescores_SVM.png')
    plt.close()
...
plt.savefig(results_path + '/average_locationS2.png')
plt.close()

#significance tests for each timepoint
all_mean_timescores = pd.read_excel(results_path + '/all_locationS2_mean_timescores_SVM.xlsx')
...
# Save the figure
plt.savefig(results_path + '/average_locationS2_with_significance.png')
plt.close()
...
plt.tight_layout()
plt.savefig(results_path + '/locationS2_violinplot.png')
plt.show()
...
plt.xlabel('Subjects')
plt.ylabel('Decoding Accuracy')
plt.title('Decoding Performance for Location Across Subjects for S2')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(results_path + '/locationS2_violinplot_avg.png')

...
data_path   = homeDir +'/AWM4_data/processed/'


# Loop over subjects:
def parallel_ica(actSubj):    
#for actSubj in Subs: 
    fname = str(homeDir)+'/AWM4_data/processed/ICAs/probe_VP'+str(actSubj)+'_fastica-ica.fif'
    #jname = str(homeDir)+'/AWM4_data/processed/ICAs/Jumps'+str(actSubj)+'.npy'
        ...
        # load cut epochs
    CombTrials = mne.read_epochs(str(homeDir)+'/AWM4_data/processed/ProbeEpochs/ProbeEpochs_VP'+str(actSubj)+'-epo-dropped.fif')
...
Parallel(n_jobs=int(num_cores/2))(delayed(parallel_ica)(actSubj) for actSubj in Subs)

...
Subs = Subs[28:] # for testing purposes
...
#for the first 7 participants the triggers were not correctly recorded, so we need to correct them
...
for actSubj in Subs:
    ...
    for ff in range(actFiles.count()):
        ...
        # if actSubj == 23 and ff == 7:
        #     events = np.insert(events, 0, [135, 0, 98], axis=0)
        #     events = np.insert(events, 1, [135, 0, 78], axis=0)
        #     events = np.insert(events, 2, [135, 0, 1], axis=0)

        # input to trigger translation file
        #trigger_stim_S2 = np.array([211 212 213 214],
                                    # [221 222 223 224],
                                    # [231 232 233 234],
                                    # [241 242 243 244])

...
# saving the results
results_path = str(homeDir)+'/AWM4_data/processed/timepoints/S1encoding(10)'
...
# all_mean_timescores = pd.read_excel(results_path + '/all_encodingS1(10)_mean_timescores_SVM.xlsx')
...
for tp, sig in significant_points:
    plt.text(timepoints[tp], mean_accuracy[tp] + 0.02, sig, color='blue', fontsize=12, ha='center')

# Save the figure
plt.savefig(results_path + '/average_encodingS1_with_significance.png')
plt.close()

# Violin plot for decoding performance across subjects for each timepoint
...
#read all_mean_acrossalltimes xls file
all_mean_acrossalltimes = pd.read_excel(results_path + '/all_encodingS1(10)_mean_acrossalltimes_SVM.xlsx')

#make a violin plot for the average decoding of all subjects for all_mean_acrossalltimes
plt.figure(figsize=(12, 8))
sns.violinplot(y='Accuracy', data=all_mean_acrossalltimes)
plt.xlabel('Subjects')
plt.ylabel('Decoding Accuracy')
plt.title('Decoding Performance Across Subjects for S1')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(results_path + '/S1decoding_violinplot_avg.png')

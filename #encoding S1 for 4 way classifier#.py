#encoding S1 for 4 way classifier#
...
# saving the results
results_path = str(homeDir)+'/AWM4_data/processed/timepoints/S1encoding(10)/4way'
...
for actSubj in sub_lst:
    ...
    #subject 23 is missing some triggers, so within all_events add 3 rows after 5845th index [:,2] being 98, 78 and 1 respectively 
    # if actSubj == 23:
    #     all_events = np.insert(all_events, 5846, [all_events[5846,0], all_events[5846,1], 98], axis=0)
    #     all_events = np.insert(all_events, 5847, [all_events[5847,0], all_events[5847,1], 78], axis=0)
    #     all_events = np.insert(all_events, 5848, [all_events[5848,0], all_events[5848,1], 1], axis=0)
    del raw, events
    ...
    #which trials are Sp1, Sp2, Sp3, Sp4
    idx_sp1 = [i for i in range(len(S1_values)) if S1_values[i] in [111, 112, 113, 114, 211, 212, 213, 214]]
    idx_sp2 = [i for i in range(len(S1_values)) if S1_values[i] in [121, 122, 123, 124, 221, 222, 223, 224]]
    idx_sp3 = [i for i in range(len(S1_values)) if S1_values[i] in [131, 132, 133, 134, 231, 232, 233, 234]]
    idx_sp4 = [i for i in range(len(S1_values)) if S1_values[i] in [141, 142, 143, 144, 241, 242, 243, 244]]
    ...
    #I will have 4 classes, so I will have 4 different y values
    y[idx_sp1] = 0
    y[idx_sp2] = 1
    y[idx_sp3] = 2
    y[idx_sp4] = 3
    ...
    np.savetxt(results_path + '/all_encodingS1(10)_mean_timescores_SVM.txt', all_mean_timescores)
    np.savetxt(results_path + '/all_encodingS1(10)_mean_acrossalltimes_SVM.txt', all_mean_acrossalltimes)

    # save as excel file with pandas
    df = pd.DataFrame(all_mean_timescores)
    df.to_excel(results_path + '/all_encodingS1(10)_mean_timescores_SVM.xlsx')

    df = pd.DataFrame(all_mean_acrossalltimes)
    df.to_excel(results_path + '/all_encodingS1(10)_mean_acrossalltimes_SVM.xlsx')
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Accuracy')
    plt.title(f'Subject {actSubj}: Decoding accuracy over time (4 classes)')
    plt.axhline(y=0.25, color='gray', linestyle='--', label='Chance Level (25%)')
    plt.axvline(x=0.5, color='r', linestyle='--') #added later to show the timepoint of the cue
    plt.savefig(results_path + f'/sub{actSubj}_encodingS1(10)_mean_timescores_SVM.png')
    plt.close()
...
plt.xlabel('Time (s)')
plt.ylabel('Accuracy')
plt.title('Average decoding accuracy over time')
...
plt.savefig(results_path + '/average_encodingS1.png')
plt.close()

#read all_mean_timescores txt file 
#all_mean_timescores = np.loadtxt(results_path + '/all_encodingS1(10)_mean_timescores_SVM.txt')
...
plt.tight_layout()
plt.savefig(results_path + '/S1decoding_violinplot.png')
#plt.show()

#!/usr/bin/env python
"""
Delay Period Sliding Window Analysis - Individual Subject Processing
For HPC cluster parallel execution
Usage: python delay_sliding_subject.py SUBJECT_ID
"""
...
def decode_sliding_window(data, y, sfreq, subject=None):
    """
    Perform decoding using a sliding window approach with proper nested cross-validation
    ...
    # Calculate number of windows using sliding approach
    # Windows slide with 10ms steps
    ...
    def process_window_nested(window_idx):
        """Process a single window with proper nested cross-validation"""
        try:
    ...
    # Run parallel processing across all windows
    print(f"\nPerforming parallel nested cross-validation across {n_windows} windows...")
    ...
    if subject is not None:
        results_path = f"{PROCESSED_DIR}/timepoints/delay_sliding_highres/params/{subject}"
...
def process_subject(subject):
    """
    Process a single subject - main function for HPC parallel execution
    
    Args:
        subject (int): Subject number
    """
    print(f"\n==== Processing subject {subject} ====")
    print(f"Using sliding window approach: {WINDOW_LENGTH_SEC*1000}ms window, {WINDOW_STEP_SEC*1000}ms steps")
    ...
    try:
        ...
        for feature_name in features:
            ...
            try:
                ...
                plt.ylim(0.45, 0.7)
                plt.tight_layout()
                plt.savefig(f'{results_path}/sub{subject}_decoding.png', dpi=150)
                plt.close()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python delay_sliding_subject.py SUBJECT_ID")
        sys.exit(1)
    ...
    os.makedirs(f"{PROCESSED_DIR}/timepoints/delay_sliding_highres", exist_ok=True)
    ...
    print(f"\nSubject {subject} processing completed successfully!")
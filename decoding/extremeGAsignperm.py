...
parser.add_argument('--n_permutations', type=int, default=100000, 
                   help='Number of permutations for significance tests (default: 100000)')
...
def compute_sign_permutation_test(individual_data, feature_name, timepoints, n_permutations=10000, alpha=0.05):
    ...
    --------
    sign_results : dict
        Dictionary containing sign permutation results
    """
    ...
    # Run permutations
    print(f"  Running {n_permutations} sign-flipping permutations...")
    for perm_idx in tqdm(range(n_permutations), desc="Sign permutations", disable=True):
...
def add_significance_markers(ax, cluster_results, sign_results, stat_method, y_position=0.57, line_width=3):
    ...
    cluster_results : dict or None
        Results from cluster permutation test
    sign_results : dict or None
        Results from sign permutation test
    stat_method : str
        Statistical method used ('cluster', 'sign', or 'both')
    y_position : float
        Base y-position for markers (default: 0.57)
    line_width : float
        Width of significance lines (default: 3)
    """
...
def add_cluster_markers(ax, cluster_results, y_position=0.57, line_width=2):
    """
    Add cluster significance markers to plot (simplified version for backward compatibility)
    ...
    cluster_results : dict or None
        Results from cluster permutation test
    y_position : float
        Y-position for markers (default: 0.57)
    line_width : float
        Width of significance lines (default: 2)
    """
...
def plot_approach_comparison(ga_data):
    ...
    if args.stat_method in ['sign', 'both']:
        ax_voice.plot([], [], color='darkred', linewidth=4, label='Sign permutation', alpha=0.8)
...
def compute_musician_level_clusters(ga_data):
    """Compute cluster and/or sign permutation tests for each musician level separately"""
    print(f"\nComputing statistical tests for each musician level using method: {args.stat_method}...")
    ...
    for approach_name, approach_info in APPROACHES.items():
        for feature in approach_info['features']:
            if feature in ga_data[approach_name]:
                ...
                # Compute statistical tests for each musician level
                musician_cluster_results = {}
                musician_sign_results = {}
                
                for level in [1, 2, 3]:
                    if level in data['musician_stats'] and data['musician_stats'][level]['n_subjects'] >= 5:
                        level_data = data['musician_stats'][level]['data']
                        
                        print(f"  Computing tests for {approach_name}_{feature}_Level_{level} (n={data['musician_stats'][level]['n_subjects']})")
                        
                        # Run cluster permutation test if requested
                        if args.stat_method in ['cluster', 'both']:
                            cluster_results = compute_cluster_permutation_test(
                                level_data, 
                        ...
                        # Run sign permutation test if requested
                        if args.stat_method in ['sign', 'both']:
                            sign_results = compute_sign_permutation_test(
                                level_data,
                                f"{approach_name}_{feature}_musician_level_{level}",
                            ...
                            musician_sign_results[level] = sign_results
                        
                    elif level in data['musician_stats']:
                        print(f"  Skipping {approach_name}_{feature}_Level_{level}: only {data['musician_stats'][level]['n_subjects']} subjects (need ≥5)")
                        if args.stat_method in ['cluster', 'both']:
                            musician_cluster_results[level] = None
                        if args.stat_method in ['sign', 'both']:
                            musician_sign_results[level] = None
                
                # Store musician statistical results
                if args.stat_method in ['cluster', 'both']:
                    ga_data[approach_name][feature]['musician_cluster_results'] = musician_cluster_results
                if args.stat_method in ['sign', 'both']:
                    ga_data[approach_name][feature]['musician_sign_results'] = musician_sign_results
...
def add_musician_cluster_markers(ax, musician_cluster_results, level, y_base=0.57, level_offset=0.02):
    ...
    if (musician_cluster_results is None or 
        level not in musician_cluster_results or 
        musician_cluster_results[level] is None):
        return
    
    cluster_results = musician_cluster_results[level]
    if cluster_results is None or 'significant_clusters' not in cluster_results:
        return
...
def add_musician_sign_markers(ax, musician_sign_results, level, y_base=0.57, level_offset=0.02):
    """Add sign permutation significance markers for specific musician level"""
    if (musician_sign_results is None or 
        level not in musician_sign_results or 
        musician_sign_results[level] is None):
        return
    
    sign_results = musician_sign_results[level]
    if sign_results is None or 'significant_clusters' not in sign_results:
        return
    ...
    for cluster in sign_results['significant_clusters']:
        ...
        ax.text(mid_time, y_position + 0.005, 
               f"L{level}", 
               ha='center', va='bottom', fontsize=6, fontweight='bold',
               bbox=dict(boxstyle='round,pad=0.1', facecolor='lightcoral', alpha=0.8))

def add_musician_significance_markers(ax, musician_cluster_results, musician_sign_results, level, stat_method):
    """Add appropriate musician-level significance markers based on statistical method"""
    if stat_method in ['cluster', 'both'] and musician_cluster_results is not None:
        add_musician_cluster_markers(ax, musician_cluster_results, level, y_base=0.57)
    
    if stat_method in ['sign', 'both'] and musician_sign_results is not None:
        y_base = 0.60 if stat_method == 'both' else 0.57
        add_musician_sign_markers(ax, musician_sign_results, level, y_base=y_base)
            ...
            if feature in ga_data[approach_name]:
                ...
                for level in [1, 2, 3]:
                    if level in data['musician_stats']:
                        ...
                        # Add individual significance markers for this musician level
                        musician_cluster_results = data.get('musician_cluster_results')
                        musician_sign_results = data.get('musician_sign_results')
                        add_musician_significance_markers(ax, musician_cluster_results, musician_sign_results, level, args.stat_method)
                
                # Add overall significance markers from grand average (at bottom)
                add_significance_markers(ax, data['cluster_results'], data['sign_results'], args.stat_method)
                ...
                if args.stat_method in ['cluster', 'both']:
                    legend_elements.extend([
                        plt.Line2D([0], [0], color='black', linewidth=3, alpha=0.8, label='Individual level clusters'),
                        plt.Line2D([0], [0], color='black', linewidth=2, alpha=0.8, label='Overall group clusters')
                    ])
                if args.stat_method in ['sign', 'both']:
                    legend_elements.extend([
                        plt.Line2D([0], [0], color='darkred', linewidth=3, alpha=0.8, label='Individual level sign perm'),
                        plt.Line2D([0], [0], color='darkred', linewidth=2, alpha=0.8, label='Overall group sign perm')
                    ])
        ...
        for level in [1, 2, 3]:
            ...
            if voice_feature and voice_feature in ga_data[approach_name]:
                ...
                if level in data['musician_stats']:
                    ...
                    # Add significance markers for this level
                    musician_cluster_results = data.get('musician_cluster_results')
                    musician_sign_results = data.get('musician_sign_results')
                    add_musician_significance_markers(ax, musician_cluster_results, musician_sign_results, level, args.stat_method)
...
def create_musician_cluster_summary_table(ga_data):
    """Create summary table of statistical results by musician level"""
    ...
    for approach_name, approach_info in APPROACHES.items():
        for feature in approach_info['features']:
            if feature in ga_data[approach_name]:
                ...
                overall_clusters = 0
                overall_cluster_duration = 0
                overall_sign_periods = 0
                overall_sign_duration = 0
                
                if data['cluster_results']:
                    overall_clusters = data['cluster_results']['n_significant']
                    overall_cluster_duration = sum([c['duration'] for c in data['cluster_results']['significant_clusters']])
                
                if data.get('sign_results'):
                    overall_sign_periods = data['sign_results']['n_significant']
                    overall_sign_duration = sum([c['duration'] for c in data['sign_results']['significant_clusters']])
                
                # Individual musician level results
                for level in [1, 2, 3]:
                    level_clusters = 0
                    level_cluster_duration = 0
                    level_sign_periods = 0
                    level_sign_duration = 0
                    level_n = 0
                    ...
                    if level in data['musician_stats']:
                        ...
                        if ('musician_cluster_results' in data and 
                            data['musician_cluster_results'] is not None and
                            ...
                            level_cluster_duration = sum([c['duration'] for c in level_cluster_results['significant_clusters']])
                        
                        # Sign permutation results
                        if ('musician_sign_results' in data and 
                            data['musician_sign_results'] is not None and
                            level in data['musician_sign_results'] and
                            data['musician_sign_results'][level] is not None):
                            
                            level_sign_results = data['musician_sign_results'][level]
                            level_sign_periods = level_sign_results['n_significant']
                            level_sign_duration = sum([c['duration'] for c in level_sign_results['significant_clusters']])
                    ...
                    statistical_summary.append({
                        ...
                        'Cluster_Duration': f"{level_cluster_duration:.3f}",
                        'N_Sign_Periods': level_sign_periods,
                        'Sign_Duration': f"{level_sign_duration:.3f}",
                        'Overall_Clusters': overall_clusters,
                        'Overall_Cluster_Duration': f"{overall_cluster_duration:.3f}",
                        'Overall_Sign_Periods': overall_sign_periods,
                        'Overall_Sign_Duration': f"{overall_sign_duration:.3f}"
                    })
    
    df = pd.DataFrame(statistical_summary)
    df.to_csv(f'{musician_dir}/musician_statistical_summary.csv', index=False)
    
    print(f"\nMusician Statistical Summary (Method: {args.stat_method}):")
    print("="*80)
    ...
    for idx, (approach_name, approach_info) in enumerate(APPROACHES.items()):
        ...
        for feature in approach_info['features']:
            if feature in ga_data[approach_name]:
                ...
                # Add significance markers
                add_significance_markers(ax_time, data['cluster_results'], data['sign_results'], args.stat_method)
            ...
            if feature in ga_data[approach_name]:
                ...
                for level in [1, 2, 3, None]:
                    if level in data['musician_stats']:
                        ...
                        # Add cluster and sign permutation info for each level
                        cluster_info = ""
                        sign_info = ""
                        ...
                        if ('musician_cluster_results' in data and 
                            ...
                            data['musician_cluster_results'][level] is not None):
                            level_clusters = data['musician_cluster_results'][level]['n_significant']
                            cluster_info = f"{level_clusters}c"
                        else:
                            cluster_info = "0c"
                        
                        if ('musician_sign_results' in data and 
                            data['musician_sign_results'] is not None and
                            level in data.get('musician_sign_results', {}) and
                            data['musician_sign_results'][level] is not None):
                            level_sign_periods = data['musician_sign_results'][level]['n_significant']
                            sign_info = f"{level_sign_periods}s"
                        else:
                            sign_info = "0s"
                        
                        if args.stat_method == 'cluster':
                            musician_cluster_info.append(f"{level_name}:{cluster_info}")
                        elif args.stat_method == 'sign':
                            musician_cluster_info.append(f"{level_name}:{sign_info}")
                        else:  # both
                            musician_cluster_info.append(f"{level_name}:{cluster_info},{sign_info}")
                ...
                summary_data.append({
                    ...
                    'Musician Clusters': ', '.join(musician_cluster_info),
...
def save_ga_data(ga_data):
    ...
    statistical_results = {}
    for approach_name, approach_info in APPROACHES.items():
        statistical_results[approach_name] = {}
        for feature in approach_info['features']:
            if feature in ga_data[approach_name]:
                statistical_results[approach_name][feature] = {}
                if ga_data[approach_name][feature]['cluster_results'] is not None:
                    statistical_results[approach_name][feature]['cluster_results'] = ga_data[approach_name][feature]['cluster_results']
                if ga_data[approach_name][feature].get('sign_results') is not None:
                    statistical_results[approach_name][feature]['sign_results'] = ga_data[approach_name][feature]['sign_results']
    
    np.save(f'{OUTPUT_DIR}/ga_statistical_results.npy', statistical_results, allow_pickle=True)
        ...
        for feature in approach_info['features']:
            if feature in ga_data[approach_name]:
                musician_results[approach_name][feature] = {
                    ...
                    'musician_sign_results': ga_data[approach_name][feature].get('musician_sign_results', {})
                }
    ...
    summary_json = {
        ...
        'statistical_parameters': {
            ...
            'cluster_test_type': 'one_sample_t_test_vs_chance',
            'sign_test_type': 'sign_flipping_permutation_vs_chance'
        },
        'statistics': {}
    }
            ...
            if feature in ga_data[approach_name]:
                ...
                for level in [1, 2, 3, None]:
                    if level in data['musician_stats']:
                        ...
                        # Add sign permutation results for this level if available
                        if ('musician_sign_results' in data and 
                            data['musician_sign_results'] is not None and
                            level in data.get('musician_sign_results', {}) and
                            data['musician_sign_results'][level] is not None):
                            ...
                            level_sign_results = data['musician_sign_results'][level]
                            level_stats['sign_results'] = {
                                'n_significant_periods': level_sign_results['n_significant'],
                                'significant_periods': level_sign_results['significant_clusters']
                            }
...
def main():
    ...
    # Compute musician-level statistical tests
    ...
    # Generate musician-specific plots with individual statistical tests
    print(f"\nGenerating musician-level analysis plots with {args.stat_method} tests...")
    ...
    # Create musician statistical summary
    print(f"\nCreating musician statistical summary...")
    ...
    print(f"Statistical method used: {args.stat_method}")

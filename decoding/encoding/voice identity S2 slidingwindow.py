#voice identity S2 slidingwindow
...
# saving the results
results_path = str(homeDir)+'/AWM4_data/processed/timepoints/S2encoding(100)/slidingWindow'
...
actSubj = 28 # change into False if you want to loop over all subjects
sub_lst = Subs.copy()
if actSubj:
...
for actSubj in sub_lst:
    ...
    #I want to drop the 63th trial for subject 28 from clean trials
    if actSubj == 28:
        CleanTrials.drop(63)
    ...
    np.savetxt(results_path + '/all_encodingS2_mean_timescores_SVM.txt', all_mean_timescores)
    np.savetxt(results_path + '/all_encodingS2_mean_acrossalltimes_SVM.txt', all_mean_acrossalltimes)

    # save as excel file with pandas
    df = pd.DataFrame(all_mean_timescores)
    df.to_excel(results_path + '/all_encodingS2_mean_timescores_SVM.xlsx')

    df = pd.DataFrame(all_mean_acrossalltimes)
    df.to_excel(results_path + '/all_encodingS2_mean_acrossalltimes_SVM.xlsx')
    ...
    plt.savefig(results_path + f'/sub{actSubj}_encodingS2_spatiotemporal_SVM.png')
    plt.close()
...
plt.savefig(results_path + '/average_encodingS2_spatiotemporal.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.title('Decoding accuracy over time')
#plt.axvline(x=1.5, color='r', linestyle='--')  # Timepoint of the cue
...
# Save the figure
plt.savefig(results_path + '/average_encodingS2_spatiotemporal_significance.png')
plt.close()
...
#read the txt file
all_mean_timescores = np.loadtxt(results_path + '/all_encodingS2_mean_timescores_SVM.txt')
all_mean_acrossalltimes = np.loadtxt(results_path + '/all_encodingS2_mean_acrossalltimes_SVM.txt')
...
# Save the figure
plt.savefig(results_path + '/average_encodingS2_spatiotemporal_significance.pdf')
plt.close()
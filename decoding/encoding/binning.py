#binned+averaged decoding analysis for AWM4 dataset
...
def decode_binned_data(data, y, sfreq, subject=None):
    """
    Perform decoding on binned data with SVM hyperparameter optimization
    ...
    for bin_idx in tqdm(range(n_bins)):
        ...
        for train_idx, test_idx in outer_cv.split(X_bin, y):
            ...
            grid_search = GridSearchCV(
                ...
                scoring='accuracy',
                n_jobs=NUM_JOBS,  # Utilize parallel processing
            ...
            accuracy = best_model.score(X_test_outer_scaled, y_test_outer)
        ...
        print(f"Bin {bin_idx+1}/{n_bins} - Best C value: {most_common_c}")
    
    # Save the best C values for reference
    results_data = np.column_stack((bin_centers, best_c_values))
    np.savetxt(f"{PROCESSED_DIR}/timepoints/PhD/bins/best_c_values.txt", 
              results_data, 
...
def save_results(all_mean_timescores, valid_subjects, bin_centers, feature_name, stimulus_name, results_path):
    ...
    detailed_path = f'{results_path}/all_{feature_name}_{stimulus_name}_byBin_SVM.xlsx'
    ...
    mean_across_bins.to_excel(f'{results_path}/all_{feature_name}_{stimulus_name}_meanAcrossBins_SVM.xlsx')
    
    # Save bin centers for reference
    pd.DataFrame(bin_centers, columns=["bin_centers"]).to_csv(f'{results_path}/bin_centers.csv', index=False)
...
def plot_results(scores, bin_centers, feature_name, stimulus_name, save_path, significant_points=None):
    ...
    if significant_points:
        for bin_idx, marker in significant_points:
            # Fix here - use the specific color for this stimulus, not the color dictionary
    ...
    plt.savefig(f'{save_path}/average_{feature_name}_{stimulus_name}_bins_significance.pdf')
    plt.savefig(f'{save_path}/average_{feature_name}_{stimulus_name}_bins_significance.png')
    plt.close()
...
def plot_comparison(results_dict, save_path, significance_dict=None):
    ...
    plt.title('Comparison of Binned Decoding Performance')
    plt.legend()
    plt.tight_layout()
    
    plt.savefig(f'{save_path}/feature_comparison_bins.pdf')
    plt.savefig(f'{save_path}/feature_comparison_bins.png')
    plt.close()
...
def run_analysis(subjects, meta_info, feature_name, stimulus_name, force_recompute_avg=False):
    ...
    results_path = f"{PROCESSED_DIR}/timepoints/PhD/bins/{feature_name}({stimulus_name})"
    ...
    for subject in subjects:
        try:
            ...
            existing_results_path = f'{results_path}/all_{feature_name}_{stimulus_name}_byBin_SVM.xlsx'
            ...
            plt.figure(figsize=(10, 8))
            plt.plot(subject_bin_centers, mean_scores, marker='o')
...
# if __name__ == "__main__": #uncomment this this later !!!!
#     # Setup
#     keyboard = Controller()
#     locale.setlocale(locale.LC_ALL, "en_US.utf8")
...
if __name__ == "__main__":
    ...
    # Check which combinations have been processed already
    completed_combinations = []
    print("Checking completed combinations...")
    for feature_name, stimulus_name in itertools.product(FEATURES.keys(), STIMULI.keys()):
        results_path = f"{PROCESSED_DIR}/timepoints/PhD/bins/{feature_name}({stimulus_name})"
        if os.path.exists(f'{results_path}/all_{feature_name}_{stimulus_name}_byBin_SVM.xlsx'):
            print(f"✓ {feature_name} during {stimulus_name} already processed")
            completed_combinations.append((feature_name, stimulus_name))
    
    # Option 1: Run only remaining combinations
    run_remaining = True
    
    # Option 2: Force regeneration of comparison plots only 
    force_comparison_plots = False
    
    if run_remaining:
        # Setup for remaining combinations
        sub_lst = Subs.copy()
        combinations = list(itertools.product(FEATURES.keys(), STIMULI.keys()))
        remaining_combinations = [c for c in combinations if c not in completed_combinations]
        
        print(f"\nProcessing remaining combinations: {remaining_combinations}")
        
        # Initialize results dictionaries for all combinations
        results_dict = {}
        significance_dict = {}
        
        # First load completed results
        for feature_name, stimulus_name in completed_combinations:
            results_path = f"{PROCESSED_DIR}/timepoints/PhD/bins/{feature_name}({stimulus_name})"
            try:
                # Load results from Excel file
                df = pd.read_excel(f'{results_path}/all_{feature_name}_{stimulus_name}_byBin_SVM.xlsx', index_col=0)
                scores = df.values
                # Load bin centers from CSV
                bin_centers = pd.read_csv(f'{results_path}/bin_centers.csv')['bin_centers'].values
                # Load significance points if available
                p_values = pd.read_csv(f'{results_path}/p_values.csv')['p_values'].values
                sig_points = []
                ...
                results_dict[(feature_name, stimulus_name)] = (scores, bin_centers)
                significance_dict[(feature_name, stimulus_name)] = sig_points
                print(f"Loaded results for {feature_name} during {stimulus_name}")
            except Exception as e:
                print(f"Could not load results for {feature_name} during {stimulus_name}: {str(e)}")
                
        # Process remaining combinations
        for feature_name, stimulus_name in remaining_combinations:
            print(f"\n==== Processing {feature_name} during {stimulus_name} ====")
            scores, bin_centers, p_values, sig_points = run_analysis(sub_lst, metaInfo, feature_name, stimulus_name)
            if scores is not None:  # Only store if we got valid results
                results_dict[(feature_name, stimulus_name)] = (scores, bin_centers)
    ...
    # Create comparison plots if we have results
    if results_dict and (force_comparison_plots or run_remaining):
        comparison_path = f"{PROCESSED_DIR}/timepoints/PhD/bins/comparisons"  
        os.makedirs(comparison_path, exist_ok=True)
        plot_comparison(results_dict, comparison_path, significance_dict)
        print("\nComparison plots created successfully!")

...
PLOT_DIR = f"{PROCESSED_DIR}/MorletTFR"
os.makedirs(PLOT_DIR, exist_ok=True)
tfr_dir = f"{PROCESSED_DIR}/MorletTFR/data"
os.makedirs(tfr_dir, exist_ok=True)

# Analysis parameters
RESAMPLE_FREQ = 200  # Hz
NUM_JOBS = 8  # CPU cores for MNE processing
...
def process_subject(subject):
    ...
    try:
        ...
        # Process temporal sensors
        temporal_sensors = SENSORS['temporal_left'] + SENSORS['temporal_right']            
        temporal_epochs = epochs.copy().pick_channels(temporal_sensors, ordered=True)
        temporal_epochs.crop(tmin=tmin, tmax=tmax)
        temporal_epochs.filter(1, None)  # High-pass filter at 1 Hz
        temporal_epochs = temporal_epochs.resample(RESAMPLE_FREQ, npad='auto')
        
        # Create full logspace but remove frequencies within ±2 Hz of 50 Hz
        freqs_full = np.logspace(np.log10(4), np.log10(100), 27)
        freqs = freqs_full[(freqs_full < 48) | (freqs_full > 52)]  # Filter out 48-52 Hz
        ...
        #temporal_tfr.apply_baseline(baseline=(-0.4, -0.25), mode='logratio')
        
        # Save TFR result
        tfr_fname = f"{tfr_dir}/sub-{subject}_temporal_tfr-nobaselinecor.h5"
        temporal_tfr.save(tfr_fname, overwrite=True)
        
        # Also save as pickle for compatibility with grand average code
        with open(f"{tfr_dir}/sub-{subject}_temporal_tfr.pkl", 'wb') as f:
            pickle.dump(temporal_tfr, f)
        
        # Plot results
        plot_tfr_single_subject(temporal_tfr, subject, subject_plot_dir)
        
        elapsed_time = time.time() - start_time
        logging.info(f"Completed Morlet TFR analysis for Subject {subject} in {elapsed_time:.2f} seconds")
        return subject, temporal_tfr
...
def plot_tfr_single_subject(tfr, subject, subject_dir):
    ...
    left_sensors = [ch for ch in SENSORS['temporal_left'] if ch in tfr.ch_names]
    if left_sensors:
        ...
        tfr.plot(
            ...
            vmin=-0.3, 
            vmax=0.3,
            title=f'Subject {subject} - Left Temporal Sensors Morlet (-0.4, -0.25 baseline)'
        )
        ...
        plt.savefig(f"{subject_dir}/left_temporal_tfr.png")
        plt.close()
    
    # Plot right temporal sensors
    right_sensors = [ch for ch in SENSORS['temporal_right'] if ch in tfr.ch_names]
    if right_sensors:
        fig = plt.figure(figsize=(10, 6))
        tfr.plot(
            picks=right_sensors,
            ...
            vmin=-0.3, 
            vmax=0.3,
            title=f'Subject {subject} - Right Temporal Sensors Morlet (-0.4, -0.25 baseline)'
        )
        plt.axvline(x=0, color='white', linestyle='--', linewidth=1, label='Stimulus 1 Onset')
        plt.axvline(x=0.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=1, color='white', linestyle='--', linewidth=1, label='Stimulus 2 Onset')
        plt.axvline(x=1.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=3.5, color='white', linestyle='--', linewidth=1, label='Ping Onset')
        plt.savefig(f"{subject_dir}/right_temporal_tfr.png")
        plt.close()
    
    # Plot frequency bands over time
    plot_frequency_bands(tfr, subject, subject_dir)

def plot_frequency_bands(tfr, subject, output_dir):
    """Plot frequency band power over time for a subject"""
    plt.figure(figsize=(10, 6))
    
    # Create a copy and apply baseline correction
    temp_tfr = tfr.copy()
    temp_tfr.apply_baseline(baseline=(-0.4, -0.25), mode='logratio')
    
    for band_name, (fmin, fmax) in FREQ_BANDS.items():
        # Find frequencies within this band
        freq_mask = (temp_tfr.freqs >= fmin) & (temp_tfr.freqs <= fmax)
        
        if not np.any(freq_mask):
            logging.warning(f"No frequencies in the {band_name} band ({fmin}-{fmax} Hz)")
            continue
        
        # Average power across frequencies in this band and across channels
        band_power = np.mean(temp_tfr.data[:, freq_mask, :], axis=(0, 1))
        
        # Plot band power over time
        plt.plot(temp_tfr.times, band_power, 
                label=f"{band_name} ({fmin}-{fmax} Hz)",
                linewidth=2)
    
    plt.axhline(0, color='k', linestyle='--')
    plt.axvline(0, color='r', linestyle='-', label='Stimulus Onset')
    plt.axvline(x=0.5, color='r', linestyle='--', linewidth=0.5, alpha=0.5, label='Stimulus Offset')
    plt.axvline(1, color='r', linestyle='-', label='Stimulus 2 Onset')
    plt.axvline(x=1.5, color='r', linestyle='--', linewidth=0.5, alpha=0.5, label='Stimulus Offset')
    plt.axvline(3.5, color='g', linestyle='-', label='Ping Onset')
...
def compute_grand_average():
    ...
    import glob
    tfr_files = sorted(glob.glob(f"{tfr_dir}/sub-*_temporal_tfr.pkl"))
...
def plot_grand_average(grand_avg, n_subjects):
    ...
    left_sensors = [ch for ch in SENSORS['temporal_left'] if ch in grand_avg.ch_names]
    if left_sensors:
        ...
        grand_avg.plot(
            ...
            vmin=-0.3, 
            vmax=0.3,
            title=f'Grand Average (N={n_subjects}) - Left Temporal Sensors Morlet (-0.4, -0.25 baseline)' 
        )
        plt.axvline(x=0, color='white', linestyle='--', linewidth=1, label='Stimulus 1 Onset')
        plt.axvline(x=0.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=1, color='white', linestyle='--', linewidth=1, label='Stimulus 2 Onset')
        plt.axvline(x=1.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=3.5, color='white', linestyle='--', linewidth=1, label='Ping Onset')
        plt.savefig(f"{grand_avg_dir}/left_temporal_tfr.png")
        plt.close()
    
    # Plot right temporal sensors grand average
    right_sensors = [ch for ch in SENSORS['temporal_right'] if ch in grand_avg.ch_names]
    if right_sensors:
        fig = plt.figure(figsize=(12, 8))
        grand_avg.plot(
            picks=right_sensors,
            ...
            vmin=-0.3, 
            vmax=0.3,
            title=f'Grand Average (N={n_subjects}) - Right Temporal Sensors Morlet (-0.4, -0.25 baseline)'
        )
        plt.axvline(x=0, color='white', linestyle='--', linewidth=1, label='Stimulus 1 Onset')
        plt.axvline(x=0.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=1, color='white', linestyle='--', linewidth=1, label='Stimulus 2 Onset')
        plt.axvline(x=1.5, color='white', linestyle='--', linewidth=0.5, label='Stimulus Offset')
        plt.axvline(x=3.5, color='white', linestyle='--', linewidth=1, label='Ping Onset')
        plt.savefig(f"{grand_avg_dir}/right_temporal_tfr.png")
        plt.close()
    
    # Plot frequency bands over time
    plot_frequency_bands(grand_avg, f"Grand Average (N={n_subjects})", grand_avg_dir)
    
    # Create 5x6 subplot figures for all subjects
    create_all_subjects_plot('left')
    create_all_subjects_plot('right')

def create_all_subjects_plot(roi='left'):
    """Create a 5x6 subplot figure with all subjects' TFR data"""
    logging.info(f"Creating combined plot for all subjects ({roi} hemisphere)...")
    ...
    fig.suptitle(f'{roi.capitalize()} Temporal Sensors - All Subjects', fontsize=16)
    
    roi_name = f'temporal_{roi}'
    ...
    for i, (subject, tfr) in enumerate(zip(subjects, all_tfrs)):
        ...
        # Get ROI sensors for this subject
        picks = [ch for ch in SENSORS[roi_name] if ch in tfr.ch_names]
        ...
        if picks:
            ...
            ax.text(0.5, 0.5, f'No data S{subject}', ha='center', va='center')
    ...
    plt.savefig(f"{PLOT_DIR}/all_subjects_{roi}_tfr.png", dpi=300)
    plt.close()
